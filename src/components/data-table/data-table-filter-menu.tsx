...
export function DataTableFilterMenu<TData>({
  ...
  // Read — mirrors getFiltersStateParser<TData> + .withDefault([])
  const filters = Array.isArray(search[filtersKey])
    ? (search[filtersKey] as ExtendedColumnFilter<TData>[])
    : []

  // Write — mirrors setFilters + clearOnDefault + shallow + throttleMs
    ...
    (
      ...
      navigate({
        search: (prev: Record<string, unknown>) => {
          ...
          return {
            ...prev,
            // clearOnDefault: true → undefined removes key from URL when empty
            [filtersKey]: next.length === 0 ? undefined : next,
          }
        },
